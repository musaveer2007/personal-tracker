...
export const Haircare = () => {
  ...
  const haircareTasks = tasks.filter(t => 
    t.name.toLowerCase().includes('haircare') &&
  ...
  return (
    <div className="pb-24 animate-fade-in">
      <div className="mb-6">
        <h1 className="text-3xl font-black tracking-tight text-white uppercase">HAIRCARE</h1>
        <p className="text-textMuted font-medium tracking-wider text-sm mt-1 uppercase">SCHEDULE-BASED SYSTEM</p>
      </div>
      
      <div className="card space-y-6">
        <h2 className="text-sm font-bold tracking-widest text-textMuted uppercase mb-4">TODAY'S ROUTINE</h2>
        <div className="space-y-4">
          {haircareTasks.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-border rounded-xl">
              <p className="text-sm text-textMuted font-bold uppercase tracking-widest">NO HAIRCARE ROUTINE SCHEDULED TODAY</p>
            </div>
          ) : (
            haircareTasks.map(task => {
